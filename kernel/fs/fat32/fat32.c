...
file_info get_file_info(u8 *name)
{
	...
	/*读取根目录*/
	root_point = (u8 *) vmalloc(pbr->bps * 512, 0);/*512unsigned char是一个标准磁盘扇区的大小，乘上每个簇包含扇区的数量得一个簇的大小*/
	...
	for (clu = pbr->rtclu; (clu & 0x0FFFFFF0) != 0x0FFFFFF0;)
	{
		...
		read_disk(sector, (u16*) root_point, pbr->bps);
		/*一簇的页目录表中逐个匹配*/
		for (table_point = 0; table_point < (pbr->bps * 512); table_point += 32)		/*目录项每项32unsigned char*/
		{
...
{
	return FAT32_DATA_CLULBA + ((clu_num - 2) * pbr->bps);
}

u32 get_next_clu(u32 clu)
{
	u32 *point, next_clu;
	point = (u32 *) vmalloc(pbr->bps * 512, 0);
...
void load_data(void *buf, u32 clu)
{
	...
	{
		sector = clu_to_sector(clu);
		read_disk(sector, (unsigned short int*) buf, pbr->bps);
		buf += (pbr->bps * 512);		/*addr指向内存中下个扇区的首地址*/
		clu = get_next_clu(clu);
	}
	return;
}
