...
void kvi_put_char(const char code)
{
	if (!kvi_sm.enable) return;
	...
	if (code < 0x20)
	{
		switch (code)
		{
			...
			case '\r':
				...
				break;
			case '\b':
				if (kvi_sm.cursor_column)
				{
					kvi_sm.cursor_column --;
				}else{
					if (kvi_sm.cursor_row)
					{
						kvi_sm.cursor_column = kvi_sm.width;
					}
				}
				break;
		}
	...
	/**如果不是控制字符*/
		...
		draw_font(kvi_sm.cursor_column * FONT_W, kvi_sm.cursor_row * FONT_H, \
			kvi_sm.fg_color, font(DEFAULT_FONT), code);
