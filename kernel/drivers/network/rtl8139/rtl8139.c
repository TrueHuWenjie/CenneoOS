...
#include <network/lwip/prot/ethernet.h>
...
PUBLIC int Rtl8139Transmit(char *buf, u32 len)
{
    ...
    if (AtomicGet(&private->txFreeNR) > 0) {
        ...
        */
        ...
        /* 传输至少60字节的数据 */
        io_out32(private->ioAddress + TX_STATUS0 + (entry * 4),
                private->txFlags | max(length, (u32_t )ETH_ZLEN));
        io_in32(private->ioAddress + TX_STATUS0 + (entry * 4)); // flush
...
PRIVATE int Rtl8139TxInterrupt(struct Rtl8139Private *private)
{
    ...
    while (AtomicGet(&private->txFreeNR) < NUM_TX_DESC) {
        ...
        /* 读取传输状态 */
        txStatus = io_in32(private->ioAddress + TX_STATUS0 + (entry * 4));
        ...
        if (txStatus & (TX_OUT_OF_WINDOW | TX_ABORTED)) {
            ...
            if (txStatus & TX_ABORTED) {
                ...
                /* 清除传输控制的abort位 */
                io_out32(private->ioAddress + TX_CONFIG, TX_CLEAR_ABT);

                /* 中断状态设置成传输错误 */
                io_out16(private->ioAddress + INTR_STATUS, TX_ERR);
                /* 写内存屏障 */
                WriteMemoryBarrier();
            }
            /* 携带丢失 */
        ...
        int status, int linkChanged)
{
    ...
    更新错过的包数
     */
	private->stats.rxMissedErrors += io_in32(private->ioAddress + RX_MISSED);
	/* 归零，下次计算 */
    io_out32(private->ioAddress + RX_MISSED, 0);
...
PRIVATE void Rtl8139RxError(u32 rxStatus,struct Rtl8139Private *private)
{
    ...
    tmp8 = io_in8(private->ioAddress + CHIP_CMD);
    /* 清除接收位 */
    io_out8(private->ioAddress + CHIP_CMD, tmp8 & ~CMD_RX_ENABLE);
    /* 再写入接收位 */
    io_out8(private->ioAddress + CHIP_CMD, tmp8);
    /* 写入接收配置 */
    io_out32(private->ioAddress + RX_CONFIG, private->rxConfig);
    private->currentRX = 0;
...
PRIVATE void Rtl8139IsrAck(struct Rtl8139Private *private)
{
    ...
    status = io_in16(private->ioAddress + INTR_STATUS) & RX_ACK_BITS;
    ...
    if (likely(status != 0)) {
        ...
        /* 写入接收回答位 */
        io_out16(private->ioAddress + INTR_STATUS, RX_ACK_BITS);
        io_in16(private->ioAddress + INTR_STATUS); // for flush
...
PRIVATE int Rtl8139RxInterrupt(struct Rtl8139Private *private)
{
    ...
    printk("In %s(), current %x BufAddr %x, free to %x, Cmd %x\n",
		    ...
		    io_in16(private->ioAddress + RX_BUF_ADDR),
            io_in16(private->ioAddress + RX_BUF_PTR),
            io_in8(private->ioAddress + CHIP_CMD));
#endif
    /* 当队列在运行中，并且接收缓冲区不是空 */
    while (!(io_in8(private->ioAddress + CHIP_CMD) & RX_BUFFER_EMPTY)) {
        /* 获取数据的偏移 */
        ...
        读取接收的状态
         */
        rxStatus = LittleEndian32ToCpu(*(u32_t *)(rxRing + ringOffset));
        /* 接收的大小在高16位 */
        rxSize = rxStatus >> 16;
...
/* 如果要打印收到的数据，就可以在此打印 */
        ...
        printk("NlltReceive(&rxRing[ringOffset + 4], pktSize);");
        ...
        io_out16(private->ioAddress + RX_BUF_PTR, (u16)(currentRX - 16));
...
PRIVATE void Rtl8139Handler(unsigned int irq, unsigned int data)
{
    ...
    /* 读取中断状态 */
    status = io_in16(private->ioAddress + INTR_STATUS);
    //io_out16(private->ioAddress + INTR_STATUS, status);
    ...
    /*if (unlikely(!netif_running(dev))) {
        // 中断屏蔽位置0，从此不接收任何中断 
        io_out16(private->ioAddress + INTR_MASK, 0);
        goto ToOut;
    }*/
	...
	if (unlikely(status & RX_UNDERRUN))
		linkChanged = io_in16(private->ioAddress + CSCR) & CSCR_LINK_CHANGE;

    /* 读取状态中非ACK和ERR的状态 */
    ackstat = status & ~(RX_ACK_BITS | TX_ERR);
	if (ackstat)    /* 有则写入该状态 */
		io_out16(private->ioAddress + INTR_STATUS, ackstat);
...
ToOut:
    ...
    printk("exiting interrupt, intr_status=%x\n",
		   io_in16(private->ioAddress + INTR_STATUS));
#endif    
    return;
}
...
PRIVATE void Rtl8139ChipReset(struct Rtl8139Private *private)
{
    /* software reset, to clear the RX and TX buffers and set everything back to defaults
        重置网卡
     */
    io_out8(private->ioAddress + CHIP_CMD, CMD_RESET);
    
    /* 循环检测芯片是否完成重置 */
    while (1) {
		Barrier();
        /* 如果重置完成，该位会被置0 */
		if ((io_in8(private->ioAddress + CHIP_CMD) & CMD_RESET) == 0)
			break;
        io_nop();
	}
...
PRIVATE int Rtl8139InitBoard(struct Rtl8139Private *private)
{
    ...
    检查丢失/损坏的硬件
     */
	if (io_in32(private->ioAddress + TX_CONFIG) == 0xFFFFFFFF) {
		printk("Chip not responding, ignoring board\n");
		return -1;
	}

    u32_t version = io_in32(private->ioAddress + TX_CONFIG) & HW_REVID_MASK;
    int i;
	...
	printk("TxConfig = 0x%x\n", io_in32(private->ioAddress + TX_CONFIG));
#endif 
	private->chipset = 0;
...
ToMatch:
	...
	if (private->chipset >= CH_8139B) {
		...
		u8_t tmp8 = io_in8(private->ioAddress + CONFIG1);
		tmp8 &= ~(SLEEP | PWRDN);
        /* 启动网卡 */
        io_out8(private->ioAddress + CONFIG1, tmp8);
	}
...
PRIVATE void __SetRxMode(struct Rtl8139Private *private)
{
	...
	printk("rtl8139_set_rx_mode(%x) done -- Rx config %x\n",
		    private->flags,
            io_in32(private->ioAddress + RX_CONFIG));
#endif 
    ...
    if (private->rxConfig != tmp) {
        /* 写入寄存器 */
        io_out32(private->ioAddress + RX_CONFIG, tmp);
        /* 刷新流水线 */
        io_in32(private->ioAddress + RX_CONFIG);
    ...
    /* 写入过滤到寄存器 */
    io_out32(private->ioAddress + MAR0 + 0, mcFilter[0]);
    io_in32(private->ioAddress + MAR0 + 0);
    
    io_out32(private->ioAddress + MAR0 + 4, mcFilter[1]);
    io_in32(private->ioAddress + MAR0 + 4);
...
PRIVATE void rtl8139HardwareStart(struct Rtl8139Private *private)
{
	...
	if (RtlChipInfo[private->chipset].flags & HAS_HLT_CLK)
		io_out8(private->ioAddress + HLT_CTL, 'R');
    ...
    解锁配置寄存器
     */
    io_out8(private->ioAddress + CFG9346, CFG9346_UNLOCK);
    io_in8(private->ioAddress + CFG9346);   // flush

    /* Restore our idea of the MAC address.
    重载mac地址
     */
    io_out32(private->ioAddress + MAC0, 
            LittleEndian32ToCpu(*(u32_t *)(private->macAddress + 0)));
    io_in32(private->ioAddress + MAC0);
    
    io_out16(private->ioAddress + MAC0 + 4, 
            LittleEndian32ToCpu(*(u16 *)(private->macAddress + 4)));
    io_in16(private->ioAddress + MAC0 + 4);
    
    private->currentRX = 0;

    /* init Rx ring buffer DMA address
    写接收缓冲区的dma地址到寄存器 */
	io_out32(private->ioAddress + RX_BUF, private->rxRingDma);
    io_in32(private->ioAddress + RX_BUF);

    /* Must enable Tx/Rx before setting transfer thresholds!
    必须在设置传输阈值前打开TX/RX功能
     */
	io_out8(private->ioAddress + CHIP_CMD, CMD_RX_ENABLE | CMD_TX_ENABLE);
    ...
    /* 把传输配置和接收配置都写入寄存器 */
    io_out32(private->ioAddress + RX_CONFIG, private->rxConfig);
    io_out32(private->ioAddress + TX_CONFIG, rtl8139TxConfig);
    ...
    if (private->chipset >= CH_8139B) {
         ...
         * 清除MAGIC位
         */
		io_out8(private->ioAddress + CONFIG3,
            io_in8(private->ioAddress + CONFIG3) & ~CFG3_MAGIC);
	}
    ...
    上锁，不可操作这些寄存器
     */
    io_out8(private->ioAddress + CFG9346, CFG9346_LOCK);
    ...
    int i;
	for (i = 0; i < NUM_TX_DESC; i++) {
        io_out32(private->ioAddress + TX_ADDR0 + (i * 4),
            private->txBuffersDma + (private->txBuffer[i] - private->txBuffers));
        /* flush */
        io_in32(private->ioAddress + TX_ADDR0 + (i * 4));
    }
    ...
    /* 没有early-rx中断 */
    io_out16(private->ioAddress + MULTI_INTR,
            io_in16(private->ioAddress + MULTI_INTR) & MULTI_INTR_CLEAR);
    
    /* 确保传输和接收都已经打开了 */
    u8_t tmp = io_in8(private->ioAddress + CHIP_CMD);
    if (!(tmp & CMD_RX_ENABLE) || !(tmp & CMD_TX_ENABLE))
        io_out8(private->ioAddress + CHIP_CMD,
                CMD_RX_ENABLE | CMD_TX_ENABLE);
    
    /* 打开已知配置好的中断 */
    io_out16(private->ioAddress + INTR_MASK, rtl8139IntrMask);
...
PRIVATE int Rtl8139Open()
{
    ...
    private->txBuffers = vmalloc(TX_BUF_TOT_LEN, VM_CTND);
    if (private->txBuffers == NULL) {
        printk("vmalloc for rtl8139 tx buffer failed!\n");
        
        return -1;
    }

    /* 分配接收缓冲区 */
    private->rxRing = vmalloc(RX_BUF_TOT_LEN, VM_CTND);
    if (private->rxRing == NULL) {
        printk("vmalloc for rtl8139 rx buffer failed!\n");
        vfree(private->txBuffers);
        return -1;
    }

    /* 把虚拟地址转换成物理地址，即DMA地址 */
    
    private->txBuffersDma = paging_virt2phys(private->txBuffers);
    private->rxRingDma = paging_virt2phys(private->rxRing);
...
PRIVATE int Rtl8139SetMacAddress(unsigned char *macAddr)
{
    ...
    /* 复制到设备 */
    memcpy(private->macAddress, macAddr, ETH_HWADDR_LEN);

    unsigned long flags = SpinLockIrqSave(&private->lock);

    /* 写入设备 */
    io_out8(private->ioAddress + CFG9346, CFG9346_UNLOCK); // 解锁
    io_in8(private->ioAddress + CFG9346);

    io_out32(private->ioAddress + MAC0 + 0, CpuToLittleEndian32(*(u32 *)(private->macAddress + 0)));
    io_in32(private->ioAddress + MAC0 + 0);
    
    io_out16(private->ioAddress + MAC0 + 4, CpuToLittleEndian16(*(u16 *)(private->macAddress + 0)));
    io_in16(private->ioAddress + MAC0 + 4);

    io_out8(private->ioAddress + CFG9346, CFG9346_LOCK); // 上锁
    io_in8(private->ioAddress + CFG9346);
...
PRIVATE int Rtl8139Close()
{
    ...
    /* 先停止命令的传输和接收 */
    io_out8(private->ioAddress + CHIP_CMD, 0);
    ...
    io_out8(private->ioAddress + CFG9346, CFG9346_UNLOCK); // 先解锁
    /* 把芯片设置回低耗模式 */
    if (RtlChipInfo[private->chipset].flags & HAS_HLT_CLK) 
        io_out8(private->ioAddress + HLT_CTL, 'H'); 
...
PRIVATE int Rtl8139InitOne(struct Rtl8139Private *private)
{
    ...
    for (i = 0; i < ETH_HWADDR_LEN; i++) {
        private->macAddress[i] = io_in8(private->ioAddress + MAC0 + i);
    }
#if RTL8139_DEBUG == 1    
     ...
     */
	if (RtlChipInfo[private->chipset].flags & HAS_HLT_CLK)
		io_out8(private->ioAddress + HLT_CTL, 'H');	/* 'R' would leave the clock running. */
...
void rtl8139_input()
{
    printk("rtl8139_input\n");
}

void rtl8139_output()
{
    printk("- - - - - - - - - - - - -rtl8139_output- - - - - - - - - - - - - - - - -\n");
}
...
PUBLIC int InitRtl8139Driver()
{
    ...
    ip_addr_t ipaddr  = IPADDR4_INIT_BYTES(120, 232, 252, 10);
    ip_addr_t netmask = IPADDR4_INIT_BYTES(255, 255, 255, 0);
    ip_addr_t gateway = IPADDR4_INIT_BYTES(120, 232, 252, 1);
    ...
    for (i = 0; i < ETHARP_HWADDR_LEN; i ++)
        netif.hwaddr[i] = private->macAddress[i];

    netif.flags = NETIF_FLAG_BROADCAST | NETIF_FLAG_ETHARP | NETIF_FLAG_LINK_UP;

    
    netif_add(&netif, &ipaddr, &netmask, &gateway, NULL, &rtl_8139_init2, &rtl8139_input);
    netif.output = &rtl8139_output;
    netif.linkoutput = &rtl8139_output;
    ...
    /* 注册并打开对应的中断 */
    register_PIC(private->irq, &Rtl8139Handler, "RTL8139 Network Controller");
    //DisableIRQ(private->irq);
