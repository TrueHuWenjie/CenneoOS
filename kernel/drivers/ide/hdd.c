...
void init_ide(void)
{
	...
	void *point;
	point = vmalloc(512, 0);
	if (point == NULL) error("IDE:memory allocate error!");		/**分配失败控制*/

	/**读取硬盘配置信息*/
	ide_get_phy_info(point);
	...
	printk("LBA_start:%#X", LBA_start);

	/**释放这个空间*/
	vfree(point);
}

void read_disk(unsigned int LBA, unsigned short *buffer, unsigned int number)
{
...
/**磁盘参数获取函数*/
static void ide_get_phy_info(struct HD_info *info)
{
