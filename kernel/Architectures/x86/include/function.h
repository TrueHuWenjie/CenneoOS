...
//读取tsc寄存器函数//
unsigned long long int tsc_read(void);
//初始化段寄存器函数
void init_seg_reg(u16 selector);
//描述符表寄存器操作函数
void x86_idtr_write(u32 base, u16 size);
u16 x86_tr_write(u16 select);
void x86_gdtr_write(u32 base, u16 size);
//GDT操作函数
u16 gdt_set(u32 base_addr, u32 length, u32 attribute);
//IDT操作函数
void idt_clean(void);
 ...
 * attribute为中断的相关属性
 */
void idt_create(u32 number, u32 selector, void *offset, u32 attribute);

// Operation of Control Register
u32 x86_cr0_read(void);
void x86_cr0_write(u32 cr0);

u32 x86_cr3_read(void);
void x86_cr3_write(u32 cr3);
