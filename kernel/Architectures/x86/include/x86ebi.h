...
/**在_start.asm中放置了loader_loader_ebi_ptr指针指向boot_info*/
extern struct boot_info *loader_ebi_ptr;
...
#define X86EBI_CHECK { \
    if (ebi.flag[0] != 'E' | \
        ebi.flag[1] != 'B' | \
        ebi.flag[2] != 'I' | \
        ebi.size != sizeof(struct boot_info)) \
        reset(); \
}
...
#define BOOT_MODE_EMBEDDED	2
#define BOOT_MODE_SERVER	3
