...
press:
	...
	/**判断鼠标点击区域*/
		...
		/**鼠标点击到了Data*/
		}else if (mouse_x > (vbe_info.xres - window_Date_length))
		{
...
void refresh_pointer(void)
{
		...
		if (mouse_x < 0)
		{
			mouse_x = (0);
			put_mouse_x(mouse_x);
		}else if (mouse_x >= vbe_info.xres)
		{
			mouse_x = (vbe_info.xres - 1);
			put_mouse_x(mouse_x);
		}
		...
		if (mouse_y < 0)
		{
			mouse_y = (0);
			put_mouse_y(mouse_y);
		}else if (mouse_y >= vbe_info.yres)
		{
			mouse_y = (vbe_info.yres - 1);
			put_mouse_y(mouse_y);
		}
